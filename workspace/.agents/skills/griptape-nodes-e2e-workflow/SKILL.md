...
description: >-
  ...
  testable section — configuration tests, error coercion checks, validation tests, and runtime
  error tests.
...
configuration, an error coercion path, a validation rule, or a runtime error condition. Workflows
are built, executed against the live engine for validation, and saved as reusable `.py` workflow
files.
...
2. **Wiki references** — consult the `griptape-nodes-e2e-wiki` skill's reference pages when
   choosing helper nodes (input providers, type converters, assertion nodes). The wiki's Quick Type
   Lookup table and parameter compatibility matrix are essential for correct wiring.
...
| `GetParameterValueRequest`       | Read back parameter values (for coercion and output checks).      |
...
| `ResolveNodeRequest`             | Execute a single node (for validation and coercion checks).       |
...
Never rely on default values alone to exercise a node. Create explicit input provider nodes
(`TextInput`, `FloatInput`, `IntegerInput`, `BoolInput`) and connect them to the target node's
inputs. This makes the test data visible in the saved workflow and easy to modify.
...
### Consult the wiki for wiring guidance

The `griptape-nodes-e2e-wiki` skill's reference pages document:

- Which assertion node accepts which types (Quick Type Lookup table).
- How to wire each node (Example Wiring section on each page).
- Which nodes serve as type converters, control flow, or utility sinks.

Read the relevant pages before designing each workflow.
...
**Choosing expected values:** The inspection report documents the parameter surface but not the
node's functional behaviour. Use the node's description (from the survey or wiki) and the input
values you chose to determine what the expected output should be. For simple pass-through nodes
(e.g. `TextInput`), the expected output equals the input. For transformation nodes (e.g.
`ToFloat`), compute the expected result. For complex nodes, choose inputs that produce predictable
outputs.
...
### Error coercion tests (`error_input_coercion`)

These verify that the node silently coerces bad input values to safe defaults at design time. This
is **not** a workflow — it is a sequence of MCP calls.

**Procedure:**

For each row in the "Input coercion" table:

1. `EnsureWorkflowAndFlowRequest` — create a fresh context.
2. `CreateNodeRequest` — create the target node.
3. `SetParameterValueRequest` — set the parameter to the bad input value from the table.
4. `GetParameterValueRequest` — read back the parameter value.
5. Verify the read-back value matches the "Actual Coerced To" column.
6. Record the result (pass/fail).
7. `ClearAllObjectStateRequest` — clean up.

If the section contains "None.", skip it entirely.

**Do not save a workflow for coercion tests.** They are design-time checks that don't produce a
meaningful workflow graph. Instead, record the results in the summary report.

### Pre-execution validation tests (`error_pre_execution_validation`)

These verify that the node rejects invalid parameter states before `process()` runs. This is also a
sequence of MCP calls, not a workflow.
...
1. `EnsureWorkflowAndFlowRequest` — create a fresh context.
2. `CreateNodeRequest` — create the target node.
3. Set parameters to trigger the validation error described in the "Condition" column.
4. `ResolveNodeRequest` — attempt to execute the node.
5. Confirm the result is a failure with an error matching the "Actual Error" column.
6. Record the result (pass/fail).
7. `ClearAllObjectStateRequest` — clean up.

If the section contains "None.", skip it entirely.

**Do not save a workflow for validation tests.** Record the results in the summary report.

### Runtime error tests (`error_runtime`)
...
Save this workflow as `<NodeType>__error_runtime_graceful.py` (or with a suffix if there are
multiple error conditions: `error_runtime_graceful_<condition>`).
...
Save this workflow as `<NodeType>__error_runtime_hard.py`.
...
Save as `<NodeType>__error_runtime.py`.
...
**Engine cleanup:** After all workflows are saved, delete the copies from the engine's workspace
directory (the originals are now in `tests/<NodeType>/`). The engine save path follows the pattern
seen in `SaveWorkflowRequest` results.
...
After all workflows are complete, clean up any `.py` files left in the engine's workspace
directory.
...
| error_input_coercion | Coercion | PASS (3/3 checks) | (no workflow) |
| error_pre_execution_validation | Validation | PASS (2/2 checks) | (no workflow) |
| error_runtime | Runtime | PASS | tests/<NodeType>/error_runtime_graceful.py, tests/<NodeType>/error_runtime_hard.py |

## Failures

<If any workflows or checks failed, list them here with error details. If all passed, write "None.">

## Engine Artifacts Cleaned

<List of files deleted from the engine workspace after copying to tests/.>
```
...
2. Determine the workflow type (configuration, coercion, validation, runtime).
3. If coercion or validation:
   a. EnsureWorkflowAndFlowRequest
   b. Run MCP call sequence (set bad values, read back, resolve)
   c. Record result
   d. ClearAllObjectStateRequest
4. If configuration or runtime:
...
5. After all sections: write summary report, clean up engine artifacts.
```
...
- **Match the inspect report's confirmed values.** The inspection report's "Actual" columns are the
  ground truth. If the report says a coercion produces `""`, assert for `""`, even if the survey
  predicted something different.

- **Library names matter.** When creating nodes via `CreateNodeRequest`, always pass
  `specific_library_name` to avoid ambiguity. The inspection report's header includes the library
  name. Assertion nodes are in `Griptape Nodes Testing Library`; input providers and utilities are
  in `Griptape Nodes Library`.
...
- **SuccessFailureNode error tests produce two workflows.** Every runtime error on a
  ...
  workflow (failure not connected). Name them with `_graceful` and `_hard` suffixes.
